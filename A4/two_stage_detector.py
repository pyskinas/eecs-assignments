...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        stem_rpn = []
        # Add the first layer, which has in_channels=in_channels
        conv = psk.get_head_stem_layer(in_channels, stem_channels[0])
        stem_rpn.append(conv)
        stem_rpn.append(nn.ReLU())

        # Add remaining layers from stem_channels
        for i in range(len(stem_channels) - 1):
            stem_rpn.append(psk.get_head_stem_layer(stem_channels[i], stem_channels[i+1]))
            stem_rpn.append(nn.ReLU())
        ...
        self.pred_obj = nn.Conv2d(stem_channels[-1], num_anchors, 1, 1) # Objectness conv
        self.pred_box = nn.Conv2d(stem_channels[-1], num_anchors*4, 1, 1) # Box regression conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, level_feats in feats_per_fpn_level.items():
            stem = self.stem_rpn(level_feats)
            object_logits[level_name] = psk.pf(self.pred_obj(stem))
            boxreg_deltas[level_name] = psk.pf(self.pred_box(stem))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            ##################################################################
            # Replace "pass" statement with your code
            # NOTE: looks like XYXY is bottom-left, top-right

            # Get anchor box details
            area = torch.tensor((stride_scale*level_stride)**2).float()
            width = torch.sqrt(area/aspect_ratio)
            height = area/width

            # Get more details about the offset from the location for each anchor corner
            x_off = width/2
            y_off = height/2

            # take away anchor box from each location
            x, y = locations.unbind(dim=1)
            anchor_boxes.append(torch.stack([x-x_off, y-y_off, x+x_off, y+y_off], dim=1))
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    # NOTE: x1y1 are bottom left, and x2y2 are top right here
    mx1, my1, mx2, my2 = boxes1.unbind(dim=1)
    nx1, ny1, nx2, ny2 = boxes2.unbind(dim=1)

    # Make boxes1 vectors into column vectors for `(v1 - v2) = matrix` type operations
    mx1 = mx1.reshape(mx1.shape[0],1)
    my1 = my1.reshape(my1.shape[0],1)
    mx2 = mx2.reshape(mx2.shape[0],1)
    my2 = my2.reshape(my2.shape[0],1)

    # Get intersections matrix. 4 cases: left, right, within, over.
    x_in = psk.get_intersection(mx1, mx2, nx1, nx2)
    y_in = psk.get_intersection(my1, my2, ny1, ny2)
    intersection = x_in*y_in
    
    # Get union matrix. 4 cases: left, right, within, over.
    m_squares = psk.get_squares(mx1, mx2, my1, my2)
    n_squares = psk.get_squares(nx1, nx2, ny1, ny2)
    union = (m_squares + n_squares) - intersection
...
def rcnn_get_deltas_from_anchors(
    ...
    deltas = None
    # Replace "pass" statement with your code
    # conver XYXY format to x, y, w, h 
    bx, by, bw, bh = psk.get_xywh(gt_boxes[:, :4])
    px, py, pw, ph = psk.get_xywh(anchors)
    ...
    # Combine delta components
    deltas = torch.stack([tx,ty,tw,th], dim=1)
    
    # Set background and neutral deltas to -1e8
...
def rcnn_apply_deltas_to_anchors(
    ...
    output_boxes = None
    # Replace "pass" statement with your code
    # Anchors have XYXY format and deltas have XYWH format (see function above).
    # Convert anchors to XYWH format and get deltas
    px, py, pw, ph = psk.get_xywh(anchors)
    tx, ty, tw, th = deltas.unbind(dim=1)

    # Get output box in xyhw
    bx = px + pw*tx
    by = py + ph*ty
    bw = pw*torch.exp(tw)
    bh = ph*torch.exp(th)
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        locations  = get_fpn_location_coords(shape_per_fpn_level=shape_per_fpn_level,
                                             strides_per_fpn_level=strides_per_fpn_level,
                                             dtype=DTYPE,
                                             device=DEVICE)

        # Run anchor function
        anchors_per_fpn_level = generate_fpn_anchors(locations_per_fpn_level=locations,
                                                     strides_per_fpn_level=strides_per_fpn_level,
        ...
        matched_gt_boxes = []
        # Replace "pass" statement with your code
        for _batch_ind in range(num_images):
            matched_boxes = rcnn_match_anchors_to_gt(anchor_boxes, 
                                         gt_boxes[_batch_ind], 
        ...
        if self.training:
            ...
            # If num_samples is -1, set it to largest possible value
            if num_samples == -1:
              num_samples = matched_gt_boxes.shape[0]
            else:
              num_samples = num_samples*self.batch_size_per_image

            # 1. Sample RPN proposals
            fg_idx, bg_idx = sample_rpn_training(matched_gt_boxes, 
                                          num_samples, 
                                          fg_prop)
            full_idx = torch.cat([fg_idx, bg_idx])

            # 2. GT delta targets
            deltas = rcnn_get_deltas_from_anchors(anchor_boxes[fg_idx], 
                matched_gt_boxes[fg_idx])
            
            matched_boxes = matched_gt_boxes[full_idx, 4]
            matched_boxes[matched_boxes >= 0] = 1
            matched_boxes[matched_boxes == -1] = 0

            # 3. Get loss
            loss_obj = F.binary_cross_entropy_with_logits(
                pred_obj_logits[full_idx], matched_boxes, reduction="none"
            )          
            loss_box = F.l1_loss(
                pred_boxreg_deltas[fg_idx], deltas, reduction="none"
            )
            ##################################################################
            #                         END OF YOUR CODE                       #
            ##################################################################
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                num_anchors = level_boxreg_deltas.shape[2]//4

                # 1. Transform Anchors
                # Get transformed boxes
                boxes = rcnn_apply_deltas_to_anchors(
                    level_boxreg_deltas[_batch_idx].reshape(-1,4), 
                    level_anchors)
                ...
                # 2. Sort proposals based on objectness score, keep `pre_nms_topk` 
                # of them.
                logits = level_obj_logits[_batch_idx].reshape(-1)
                pre_nms_topk_logits, pre_nms_topk_ind = torch.topk(
                    logits, 
                    k = min(self.pre_nms_topk,logits.shape[0]))
                pre_nms_topk_boxes = boxes[pre_nms_topk_ind]

                # 3. Apply nms to remaining boxes
                # get nms indices
                idx = torchvision.ops.nms(pre_nms_topk_boxes,
                                        pre_nms_topk_logits,
                                        self.nms_thresh)
                level_proposals_per_image.append(
                    pre_nms_topk_boxes[idx]
                )
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        # Get number of channels in the FPN layer
        in_channels = self.backbone.out_channels
        
        # Make the stem.
        # Add the first layer here, it has non indexible in_channels
        first_layer = psk.get_head_stem_layer(in_channels, stem_channels[0])
        cls_pred.append(first_layer)
        cls_pred.append(nn.ReLU())

        # Add remaining layers by indexing stem_channels
        for i in range(1, len(stem_channels)):
            cls_pred.append(psk.get_head_stem_layer(stem_channels[i-1], stem_channels[i]))
            cls_pred.append(nn.ReLU())
        ######################################################################
        ...
        # Linear layer. W ~ N(0, 0.01), b = 0.
        in_channels = stem_channels[-1]*roi_size[0]*roi_size[1]
        linear = nn.Linear(in_channels, num_classes+1)
        linear.weight = nn.Parameter(torch.randn_like(linear.weight)*0.01)
    ...
    def forward(
        ...
        NOTE: pyskinas, `proposals_per_fpn_leve` is never really used, so if you get
        bad training consider using it.
        """
        ...
        # NOTE: There is a relationship between fg_prop and num samples. 
        # As fg_prop goes down, num_samples goes up. This is because most
        # matched boxes are background boxes
        fg_prop = 0.2
        num_samples = 20 
        ...
        # Get display of ration of fg to bg crops, as num_samples goes up, you
        # get proportionally more bg images up to a limit of around 1:19.
        # fg_num = len(fg_idx)
        # bg_num = len(bg_idx)
        # bg_prop = bg_num/fg_num
        # ideal_bg_prop = (1-fg_prop)/fg_prop
        # print(f"Ratio of foreground to background is 1:{bg_prop} ;  should be 1:{ideal_bg_prop}")
        ...
        # 2. get class labels
        input_cls = pred_cls_logits[full_idx]
        target_cls = matched_gt_boxes[full_idx, 4] + 1
    ...
    def inference(
        ...
        pred_scores = softmax(pred_cls_logits)

        # Get classes from scores
        pred_scores, pred_classes = torch.max(pred_scores, dim=1)
        
        # Remove background 
        pred_classes = pred_classes - 1
        keep_idx = (pred_classes != -1) & (pred_scores >= test_score_thresh)
