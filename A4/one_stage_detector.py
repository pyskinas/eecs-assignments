...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        cls_conv = self.get_head_stem_layer(in_channels, stem_channels[0])
        box_conv = self.get_head_stem_layer(in_channels, stem_channels[0])
        ...
        for i in range(len(stem_channels) - 1):
            stem_cls.append(self.get_head_stem_layer(stem_channels[i], stem_channels[i+1]))
            stem_cls.append(nn.ReLU())
            stem_box.append(self.get_head_stem_layer(stem_channels[i], stem_channels[i+1]))
            stem_box.append(nn.ReLU())
    ...
    def get_head_stem_layer(self, in_channels, out_channels):
        """
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        # Go through each layer in the pyramid
        for fpn_layer, fpn_out in feats_per_fpn_level.items():
            # Get classification scores
            cls_stem_out = self.stem_cls(fpn_out)
            class_logits[fpn_layer] = self.pf(self.pred_cls(cls_stem_out))
            
            # Get box regression & centerness
            box_stem_out = self.stem_box(fpn_out)
            boxreg_deltas[fpn_layer] = self.pf(self.pred_box(box_stem_out))
            centerness_logits[fpn_layer] = self.pf(self.pred_ctr(box_stem_out))
...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        x, y = centers.unsqueeze(dim=2).unbind(dim=1) # columns
        x0, y0, x1, y1 = gt_boxes[:, :4].unsqueeze(dim=0).unbind(dim=2) # rows
        pairwise_dist = torch.stack([x - x0, y - y0, x1 - x, y1 - y], dim=2) # columns x rows
        ...
        # Interesting, different levels look for differnt scales
...
def fcos_get_deltas_from_locations(
    ...
    x1, y1, x2, y2 = torch.unbind(gt_boxes[:, :4], dim=1)
...
def fcos_apply_deltas_to_locations(
    ...
    # Un-normalised
    l, t, r, b = torch.unbind(deltas*stride, dim=1)
    output_boxes = torch.stack([xc -l, yc-t, xc+r, yc+b], dim=1).clip(min=0)

    # (-1, -1, -1, -1) deltas are made into corresponding feature locations
    output_boxes[deltas < 0] = torch.stack([xc, yc, xc, yc], dim=1)[deltas<0]
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = None
    # Replace "pass" statement with your code
    centerness = (torch.min(deltas[:,[0,2]], dim=1).values * torch.min(deltas[:,[1,3]], dim=1).values)/ (
        torch.max(deltas[:,[0,2]], dim=1).values *  torch.max(deltas[:,[1,3]], dim=1).values
    )
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        # Initialise backbone FPN
        self.backbone = DetectorBackboneWithFPN(fpn_channels)

        # Initialise prediction network (head)
    ...
    def forward(
        ...
        ######################################################################
        # Pass through backbone, returns dict {"p3", "p4", "p5"}
        fpn_feats = self.backbone(images) 

        # Get prediction head output returns an array of dicts
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = self.pred_net(fpn_feats)

        # Get device
        DEVICE = fpn_feats["p3"].device
        ######################################################################
        ...
        ######################################################################
        fpn_feat_shapes = {key:value.shape for key, value in fpn_feats.items()}

        locations_per_fpn_level = get_fpn_location_coords(fpn_feat_shapes,
    ...
    def eliminate(self, x : torch.Tensor, ind: torch.Tensor):
      shape = x.shape
      x[ind] = -3
      if len(shape) == 1:
          return x[x != -3]
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 2:
            elim_ind = level_pred_scores < test_score_thresh # Set background class to -1
            level_pred_scores = self.eliminate(level_pred_scores, elim_ind)
            level_pred_classes = self.eliminate(level_pred_classes, elim_ind)

            # Step 3:
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas,
                                                              level_locations,
                                                              self.backbone.fpn_strides[level_name])
            level_pred_boxes = self.eliminate(level_pred_boxes, elim_ind)

            # Step 4: Use `images` to get (height, width) for clipping.
            _, _, H, W = images.shape
            level_pred_boxes = level_pred_boxes.clip(min=0)
            level_pred_boxes[:,[0,2]] = level_pred_boxes[:,[0,2]].clip(max=W) # clipping x coordinates
            level_pred_boxes[:,[1,3]] = level_pred_boxes[:,[1,3]].clip(max=H) # clipping y coordinates
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
