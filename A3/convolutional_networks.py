...
from fully_connected_networks import sgd, Linear_ReLU, Linear, Solver, adam, ReLU, sgd_momentum
...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        pad = conv_param["pad"]
        pad = (pad, pad, pad, pad)
        x_temp = torch.nn.functional.pad(x, pad, "constant", 0)
        pad = pad[0]
        ...
        out = torch.zeros(N, F, new_H, new_W, device=x.device, dtype=x.dtype)

        for k in range(N):
          for f in range(F):
            for i in range(new_H):
              for j in range(new_W):
                rf = x_temp[k,:, i*stride: HH +i*stride, j*stride:WW+j*stride]
                out[k, f, i, j] = torch.sum(rf*w[f,:, :, :]) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        # re-pad x for simplicity - padding x (input)
        pad = conv_param["pad"]
        pad = (pad, pad, pad, pad)
        x_temp = torch.nn.functional.pad(x, pad, "constant", 0)
        pad = pad[0]

        # Bias backprop is fairly straight-forward so we implement it here
        db = torch.sum(dout, dim=(0,2,3))

        # Initialise gradient matrices
        dw = torch.zeros_like(w)
        dx = torch.zeros_like(x_temp)

        for k in range(N):
          for f in range(F):
            for i in range(new_H):
              for j in range(new_W):
                rf = x_temp[k, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
                dw[f, :, :, :] += rf*dout[k,f,i,j]
                dx[k, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += w[f, :, :, :]*dout[k,f,i,j]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros(N, C, new_H, new_W, device=x.device, dtype=x.dtype)

        
        for i in range(new_H):
            for j in range(new_W):
                val, ind = torch.max(x[:, :, i*s:i*s+ph, j*s:j*s+pw], 2)
                val, ind = torch.max(val, 2)
                out[:, :, i, j] = val
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
    ...
    def backward(dout, cache):
        ...
        dx = torch.zeros(N, C, H, W, device=dout.device, dtype=dout.dtype)

        for k in range(N):
            for c in range(C):
                for i in range(new_H):
                    for j in range(new_W):
                        rf = x[k, c, i*s:i*s+ph, j*s:j*s+pw]
                        val, ind1 = torch.max(rf, 1)
                        val, ind2 = torch.max(val,0)
                        ind = ind1[ind2]
                        val = dout[k, c, i, j]
                        dx[k, c, i*s+ind2, j*s+ind] = val
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Convolutional
        self.params["W1"] = torch.randn((num_filters, 3, filter_size, filter_size), device=device, dtype=dtype)*weight_scale
        self.params["b1"] = torch.zeros(num_filters, device=device, dtype=dtype)
        
        # Linear
        W2_size = (num_filters*input_dims[1]*input_dims[2])//4
        self.params["W2"] = torch.randn((W2_size, hidden_dim), device=device, dtype=dtype)*weight_scale
        ...
        self.params["b3"] = torch.randn(num_classes, device=device, dtype=dtype)       
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def loss(self, X, y=None):
        ...
        N = X.shape[0]
        cache = {}
        scores, cache["L1"] = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)

        # Flatten?
        conv_out_shape = scores.shape
        scores = scores.reshape(N, -1) 

        scores, cache["L2"] = Linear_ReLU.forward(scores, W2, b2)
        scores, cache["L3"] = Linear.forward(scores, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dout, grads["W1"], grads["b1"] = Conv_ReLU_Pool.backward(dout, cache["L1"])
        

        loss += self.reg*(torch.sum(W3*W3) + torch.sum(W2*W2))
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(1, self.num_layers):
            ...
            # Do we want to use batchnorm?
            if batchnorm:
              # Batch normalisation scale and shift params
              self.params["bn"+I+"_scale"] = torch.ones(num_filters[i-1], device=device, dtype=dtype)
              self.params["bn"+I+"_shift"] = torch.zeros(num_filters[i-1], device=device, dtype=dtype)
            
            # Initialise weights
            # Kaiming?
            if weight_scale=="kaiming":
                if i == 1:
                    self.params["W1"] = kaiming_initializer(input_dims[0], num_filters[i-1], K=3, device=device, dtype=dtype)
                else:
                    self.params["W"+I] = kaiming_initializer(num_filters[i-2], num_filters[i-1], K=3, device=device, dtype=dtype)
            else:
                if i == 1:
                    self.params["W1"] = torch.randn((num_filters[i-1], input_dims[0], 3, 3), device=device, dtype=dtype)*weight_scale
                else:
                    self.params["W"+I] = torch.randn((num_filters[i-1], num_filters[i-2], 3, 3), device=device, dtype=dtype)*weight_scale
            # Initialise biases
            self.params["b"+I] = torch.zeros(num_filters[i-1], device=device, dtype=dtype)
        
        # Linear Layer
        flattened_dim = num_filters[-1]*(input_dims[1]*input_dims[2])//(4**len(max_pools))
        if weight_scale=="kaiming":
            self.params["W"+str(self.num_layers)] = kaiming_initializer(flattened_dim, num_classes, device=device, dtype=dtype)
        else:
            self.params["W"+str(self.num_layers)] = torch.randn((flattened_dim, num_classes), device=device, dtype=dtype)*weight_scale
    ...
    def loss(self, X, y=None):
        ...
        pm = self.params
        scores = X
        hasBN = self.batchnorm # Do we have batchnorm
        ...
        for i in range(self.num_layers-1):
            # Stringify index+1 to get params dict keys.
            I = str(i+1)

            # Are we pooling?
            if i in self.max_pools:
                # Pool
                # Are we batch normalising?
                if hasBN:
                    # Batchnorm
                    scores, cache["L"+I] = Conv_BatchNorm_ReLU_Pool.forward(scores, 
                                                                            pm["W"+I],
                                                                            pm["b"+I], 
                                                                            pm["bn"+I+"_scale"], 
                                                                            pm["bn"+I+"_shift"], 
                  ...
                  # Don't Batchnorm
                  scores, cache["L"+I] = Conv_ReLU_Pool.forward(scores, pm["W"+I], pm["b"+I], conv_param, pool_param)
            else:
                # Don't pool
                # Are we batch normalising?
                if hasBN:
                    # Batchnorm
                    scores, cache["L"+I] = Conv_BatchNorm_ReLU.forward(scores, pm["W"+I], pm["b"+I],
                                                                            pm["bn"+I+"_scale"], 
                                                                            pm["bn"+I+"_shift"],  
                                                                            conv_param,
                                                                            bn_params[i] )
                    ...
                    # Don't Batchnorm
                    scores, cache["L"+I] = Conv_ReLU.forward(scores, pm["W"+I], pm["b"+I], conv_param)
        
        # Linear layer.
        # Flatten output
        conv_out_shape = scores.shape
        scores = scores.reshape(scores.shape[0], -1)
        # Final pass through linear layer
        I = str(self.num_layers)
        scores, cache["L"+I] = Linear.forward(scores, pm["W"+I], pm["b"+I])
        ...
        dout, grads["W"+I], grads["b"+I] = Linear.backward(dout, cache["L"+I])

        # Regularisation for linear layers
        loss += self.reg*torch.sum(pm["W"+I]*pm["W"+I])
        grads["W"+I] += 2*self.reg*pm["W"+I]
        ...
        for i in range(self.num_layers-1, 0, -1):
          ...
          if i-1 in self.max_pools:
            ...
            if hasBN:
              # Batchnormed
              dout, grads["W"+I], grads["b"+I], grads["bn"+I+"_scale"], grads["bn"+I+"_shift"] = Conv_BatchNorm_ReLU_Pool.backward(dout, cache["L"+I])
            else:
              # Did not Batchnorm
              dout, grads["W"+I], grads["b"+I] = Conv_ReLU_Pool.backward(dout, cache["L"+I])
          else:
            # Did not pool
            # Did we Batchnorm?
            if hasBN:
              # Batchnormed
              dout, grads["W"+I], grads["b"+I], grads["bn"+I+"_scale"], grads["bn"+I+"_shift"] = Conv_BatchNorm_ReLU.backward(dout, cache["L"+I])
            else:
              # Did not Batchnorm
              dout, grads["W"+I], grads["b"+I] = Conv_ReLU.backward(dout, cache["L"+I])
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # and device.                                                     #
        ###################################################################
        weight = torch.randn((Din, Dout), device=device, dtype=dtype)*((gain/Din)**(1/2))
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        # and device.                                                     #
        ###################################################################
        weight = torch.randn((Dout, Din, K, K), device=device, dtype=dtype)*((gain/(Din*K*K))**(1/2))
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            sqvar = torch.sqrt(var+eps)
            out = xmu/sqvar

            # Cache
            cache = (x, out, mean, var, sqvar, gamma, bn_param)
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        x, out, mean, var, sqvar, gamma, bn_param = cache
        N, _ = x.shape
        eps = bn_param.get("eps", 1e-5)

        dgamma = torch.sum(out*dout, dim=0)
        ...
        # # x - mean
        # xmm = x-mean
        # dvar = -0.5*torch.sum(dx_hat*xmm*torch.pow((var+eps), -1.5), dim=0)
        # temp_var = 1/(torch.sqrt(var + eps))
        # temp_m = (2/N)*xmm
        # temp = dx_hat*temp_var
        # temp += temp_m*dvar
        # dm = -torch.sum(temp, dim=0)
        # dx = temp + dm/N

        # Old version of my algebraic way - terribley
        # num = x - mean
        # den = torch.sqrt(var + eps)
        # part1 = (1- 1/N)*(gamma/den)
        
        
        # v1 = torch.pow(var, -1.5)
        # v2 = 2*torch.sum(num, dim=0)
        # v2 /= N
        # v2 = -num + v2
        # v3 = v1*v2
        # v3 /= 2*N
        ...
        # My computational graph way - same output as paper method
        dx = dout*gamma
        dm1 = -torch.sum(dx*(1./torch.sqrt(var + eps)), dim=0)
        dv = torch.sum(-0.5*dx*(x-mean)*torch.pow(var+eps, -1.5), dim=0)
        dm2 = dv*((-2./N)*torch.sum(x-mean, dim=0))
        dm = dm1+dm2
        dx1 = (torch.zeros_like(dx)+dm)/N
        dx2 = (2./N)*(x-mean)*dv
        dx3 = dx*(1./torch.sqrt(var+eps))
        dx = dx1+dx2+dx3
    ...
    def backward_alt(dout, cache):
        ...
        dbeta = torch.sum(dout, dim=0)
        nest = out*dgamma
        new = torch.sum(nest, dim=0)

        dx = (N*dout - dbeta - nest + new/N)*(gamma/(sqvar*N))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        ################################################################
        N, C, H, W = x.shape
        temp = torch.reshape(torch.permute(x, (0,2,3,1)), (-1, C))
        out, cache = BatchNorm.forward(temp, gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        #################################################################
        N, C, H, W = dout.shape
        temp = torch.reshape(torch.permute(dout, (0,2,3,1)), (-1,C))
        dx, dgamma, dbeta = BatchNorm.backward(temp, cache)
