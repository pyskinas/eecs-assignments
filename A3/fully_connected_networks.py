...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        params = self.params
        h1, L1_cache = Linear_ReLU.forward(X, params["W1"], params["b1"])
        scores, L2_cache = Linear.forward(h1, params["W2"], params["b2"])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dout, grads["W2"], grads["b2"] = Linear.backward(dout, L2_cache)
        dout, grads["W1"], grads["b1"] = Linear_ReLU.backward(dout, L1_cache)

        loss += self.reg*(torch.sum(params["W1"]*params["W1"]) + torch.sum(params["W2"]*params["W2"]))
        grads["W2"] += 2*self.reg*params["W2"]
        grads["W1"] += 2*self.reg*params["W1"]
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Makes things easier
        new_list = [input_dim] + hidden_dims + [num_classes]
        
        for i in range(1, len(new_list)):
            self.params["W" + str(i)] = torch.randn(new_list[i-1], new_list[i], dtype=dtype, device=device)*weight_scale
            self.params["b" + str(i)] = torch.zeros(new_list[i], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            ...
            if i == self.num_layers:
                cache["L"+I], cache["L"+I+"_cache"] = Linear.forward(scores, self.params["W"+I], self.params["b"+I])
            else:
                cache["L"+I], cache["L"+I+"_cache"] = Linear_ReLU.forward(scores, self.params["W"+I], self.params["b"+I])
            
            # Use Dropout?
            if self.use_dropout and i != self.num_layers:
              scores, cache["L"+I+"_drop"] = Dropout.forward(cache["L"+I], self.dropout_param)
            else:
              scores = cache["L"+I]
        #################################################################
        #                      END OF YOUR CODE                         #
        ...
        for i in range(self.num_layers, 0, -1):
            I = str(i)
            if i == self.num_layers:
                dout, grads["W"+I], grads["b"+I] = Linear.backward(dout, cache["L"+I+"_cache"])
            else:
                dout, grads["W"+I], grads["b"+I] = Linear_ReLU.backward(dout, cache["L"+I+"_cache"])

            # Use dropout?
            if self.use_dropout and i != 1:
                dout = Dropout.backward(dout, cache["L"+str(i-1)+"_drop"])


            # Regularisation
            loss += self.reg*torch.sum(self.params["W"+I]*self.params["W"+I])
            grads["W"+I] += 2*self.reg*self.params["W"+I]
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def rmsprop(w, dw, config=None):
    ...
    d = config["decay_rate"]
    grad_squared = d*config["cache"] + (1-d)*dw*dw
    w -= lr * dw/(grad_squared.sqrt() + config["epsilon"])
    next_w = w
...
def adam(w, dw, config=None):
    ...
    eps = config["epsilon"]
    lr = config["learning_rate"]
    t = config["t"] if config["t"] > 0 else 5

    # Algorithm
    m1 = b1*config["m"] + (1-b1)*dw
    m2 = b2*config["v"] + (1-b2)*dw*dw
    m1u = m1/(1-b1**t)
    m2u = m2/(1-b2**t)

    # Update
    w -= lr*m1u/(m2u.sqrt() + eps)
