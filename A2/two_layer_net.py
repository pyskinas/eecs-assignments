...
def nn_forward_backward(
    ...
    m,_ =scores.max(dim=1)
    s = (scores.t() - m).t()

    # Get softmaxed scores
    sm = s.exp()
    S = sm.sum(dim=1)
    sm = (sm.t()/S).t()

    # Get the loss
    loss = -torch.sum(torch.log(sm[torch.arange(N), y]))
    loss/=N
    ...
    # tensor of same size                                                     #
    ###########################################################################
    # W2; W2rv1 = W2 related variable 1
    H, C = W2.shape
    W2rv1 = torch.zeros_like(sm)
    W2rv1[torch.arange(N),y] = 1
    grads["W2"] = h1.t().mm(sm - W2rv1)/N
    grads["W2"] += 2*reg*W2
    
    # b2
    grads["b2"] = torch.sum(sm - W2rv1, dim=0)/N
    
    # derivative w.r.t. h-hat
    dh = (sm - W2rv1).mm(W2.t())
    dh[h1<=0] = 0
...
def nn_get_search_params():
    ...
    # classifier.                                                             #
    ###########################################################################
    learning_rates = [1.08, 1.1, 1.12]
    hidden_sizes = [3000]
    regularization_strengths = [6e-4, 7e-4, 9e-4]
    learning_rate_decays = [0.96]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    lrates, hsizes, regs, lrdecays = get_param_set_fn()
    
    # get input and output dims
    input_dim = data_dict["X_train"].shape[1]
    output_dim = torch.max(data_dict["y_train"]).item() + 1 # 10  hardcoded for cifar-10 (TODO ?)

    # For counting the combination you're on
    count = 1
    total = len(lrates)*len(hsizes)*len(regs)*len(lrdecays)

    for lr in lrates:
      for size in hsizes:
        for reg in regs:
          for decay in lrdecays:
            net = TwoLayerNet(input_dim, size, output_dim, dtype=data_dict["X_train"].dtype, device=data_dict["X_train"].device)
            ...
            if acc > best_val_acc:
              ...
              best_stat = stats
              best_net = net
            print(f"{count} of {total} |lr: {lr}| size: {size}| reg: {reg}| decay: {decay}| accuracy: {acc}")
            count +=1
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
