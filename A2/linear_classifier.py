...
from matplotlib.legend import Rectangle
import torch
import random
import statistics
...
def svm_loss_vectorized(
    ...
    K[K == 1] = 0
    K[K < 0] = 0
    loss = torch.sum(K)/N + reg*torch.sum(W*W)
    ...
    #############################################################################
    dK = (K>0)
    t1 = -torch.sum(dK, dim = 1)
    t2 = torch.zeros_like(dK)
    t2[torch.arange(N), y] = 1
    t2 = (t2.t() * t1).t()
    dS = (dK + t2).double()
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    ind = torch.randperm(num_train)[:batch_size]
    X_batch = X[ind]
    y_batch = y[ind]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [8e-3, 1e-2, 3e-2, 5e-2]
...
def softmax_loss_naive(
    ...
    for i in range(N):
      ...
      # dW += X[i].reshape(D,1).mm(scores.reshape(1,C))
      # dW[:, y[i]] -= X[i]
      for j in range(C):
        if j == y[i]:
          dW[:,j] += (-1+scores[y[i]])*X[i]
        else:
          dW[:,j] += scores[j]*X[i]   
    loss /= N
    loss += reg*torch.sum(W*W)
...
def softmax_loss_vectorized(
    ...
    N = X.shape[0]
    l = X.mm(W)
    temp, _ = torch.max(l, dim=1)
    l = (l.t() - temp).t()
    el = torch.exp(l) # exponentiated logits
    sums = torch.sum(el, dim=1)
    scores = (el.t().div(sums)).t() # our scores (NxC)
    loss = torch.sum(-torch.log(scores[torch.arange(N), y]))/N

    yhat = torch.zeros_like(scores) # Matrix, NxC, where yhat[i][y[i]] = 1, else = 0
    yhat[torch.arange(N), y] = 1
    dW = X.t().mm(scores - yhat)/N
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [3e-2, 3.6e-2, 4.5e-2, 5.51e-2, 6.75e-2]
    regularization_strengths = [1e-5, 3e-5, 1e-4, 3e-4, 1e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
