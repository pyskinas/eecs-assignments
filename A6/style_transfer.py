...
def content_loss(content_weight, content_current, content_original):
    ...
    c = content_current.clone().squeeze().reshape(C, -1) # current
    o = content_original.clone().squeeze().reshape(C, -1) # original
    cl = content_weight * torch.sum((c - o)**2)
    return cl
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, l in enumerate(style_layers):
       ...
       style_loss += weight*torch.norm(gram - target, p=2)**2
...
def tv_loss(img, tv_weight):
    ...
    # Vectorized 
    h_loss = torch.sum((img[:, :, 1:] - img[:, :, :H-1])**2)
    w_loss = torch.sum((img[:, :, :, 1:] - img[:, :, :, :W-1])**2)
    
    # # Using 2 loops - debuge
    # # This works, it was used to test whether I understood
    # # the underlying concept. After this I implemented the 
    # # vectorised version correctly.
    # h_loss = 0
    # w_loss = 0
    # for i in range(H):
    #    for j in range(W):
    ...
    #         continue
    #       elif i == H - 1:
    #         w_loss += (img[0, :, i, j+1] - img[0, :, i, j])**2
    #       elif j == W - 1:
    #         h_loss += (img[0, :, i+1, j] - img[0, :, i, j])**2
    #       else:
    #         h_loss += (img[0, :, i+1, j] - img[0, :, i, j])**2
    #         w_loss += (img[0, :, i, j+1] - img[0, :, i, j])**2
    # h_loss = torch.sum(h_loss)
    # w_loss = torch.sum(w_loss)
          

    return tv_weight*(w_loss + h_loss)
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
