...
def compute_saliency_maps(X, y, model):
    ...
    out = model(X)
    N, C = out.shape
    loss = torch.sum(out[torch.arange(N), y[torch.arange(N)]])
    loss.backward()

    saliency = X.grad.data.permute(0,2,3,1)
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # after each step.                                                     #
    ########################################################################
    # We need the gradient WRT the image.
    img = img.requires_grad_()

    # Get the output.
    out = model(img)

    # Get the Loss, sy(I) - reg*p2norm
    loss = out[0, target_y] - l2_reg*torch.norm(out, p=2)**2
    ...
    # Update image - gradient ascent step
    img = img.detach() + learning_rate*grad
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
    return img
