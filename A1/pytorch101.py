...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    new_stop = (stop + 1) + (10 - (stop+1) % 10)
    x = torch.arange(new_start, new_stop, 10).to(torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    tensors = []
    for i in range(x.shape[0]):
        tensors.append(x[i].mm(y[i]))
    z = torch.stack(tensors, dim=0)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    m = x.sum(dim=0)/x.shape[0]
    s = torch.sqrt(torch.sum((x-m)**2/(x.shape[0] - 1), dim=0))
    y = ((x-m)/s).clone()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
