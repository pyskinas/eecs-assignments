...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        train = x_train[i].view(-1)
        for j in range(num_test):
            dists[i, j] = torch.sum((train - x_test[j].view(-1))**2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_test_flat = x_test.view(num_test, -1).clone()
    # print(x_test_flat.shape)
    for i in range(num_train):
        dists[i] = torch.sum((x_test_flat - x_train[i].view(-1))**2, dim=1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    a1 = torch.sum(x_train.view(num_train, -1)**2, dim=1).view(-1, 1)
    # print(a1.shape)
    a2 = torch.sum(x_test.view(num_test, -1)**2, dim=1).view(1, -1)
    # print(a2.shape)
    a = a1 + a2
    # print(a.shape)
    b = x_train.view(num_train, -1).mm(x_test.view(num_test, -1).t())
    dists = a - 2*b
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, k_ind = torch.topk(dists, k, dim=0, largest=False)
    # print(k_ind)
    topk = y_train[k_ind]
    # print(topk)
    topkmode, _ = torch.mode(topk, dim=0)
    # print(topkmode)
    y_pred = topkmode
    ##########################################################################
    #                           END OF YOUR CODE                             #
        ...
        for i in range(num_folds):
            if i == num_folds-1:
                new_x_train = torch.cat(
                    (x_train_folds[:i]), dim=0)
                new_y_train = torch.cat(
                    (y_train_folds[:i]), dim=0)
            else:
                new_x_train = torch.cat(
                    (x_train_folds[:i] + x_train_folds[-(num_folds-i-1):]), dim=0)
                new_y_train = torch.cat(
                    (y_train_folds[:i] + y_train_folds[-(num_folds - i - 1):]), dim=0)
            new_x_test = x_train_folds[i]
            new_y_test = y_train_folds[i]
            model = KnnClassifier(x_train=new_x_train, y_train=new_y_train)
            k_to_accuracies[k].append(model.check_accuracy(
                new_x_test, new_y_test, k=k, quiet=True))               #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
        mean = torch.mean(torch.tensor(k_to_accuracies[k])).item()
        if mean > best_acc:
            best_k = k
            best_acc = mean
    ##########################################################################
